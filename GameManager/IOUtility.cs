...
namespace GameManager {
    ...
    public static class IOUtility {
        ...
        public static bool IsRpgMakerGame(string path) {
            ...
            if (IOUtility.EnumerateFilesSafely(gameDir, "System.json", System.IO.SearchOption.AllDirectories).FirstOrDefault() != null)
            {
                return true;
            }

            foreach(string file in EnumerateFilesSafely(gameDir, "*.exe"))
            {
                FileStream fileStream = File.Open(file, FileMode.Open, FileAccess.Read);
                BinaryReader binaryReader = new BinaryReader(fileStream);
                byte[] chunk = binaryReader.ReadBytes(1024);
                int pos = GetPosition(chunk, ASCIIEncoding.ASCII.GetBytes("enigma"), 0);
                fileStream.Close();
                if (pos > 1)
                {
                    return true;
        ...
        /// </summary>
        public static byte[] baXOR(byte[] arr1, byte[] arr2)
        {
        ...
        public static int GetPosition(byte[] data, byte[] pattern, int startIndex)
        {
