        ...
        public virtual string GetCategory() {
            if (html != null) {
                ...
                if (tagIndex > 0 && tagLength > 0) {
					tagPrefix = "for=\"work_type_" + html.Substring(tagIndex, tagLength) + "\">";
					tagSuffix = "</label>";
					tagIndex = htmlfs.IndexOf(tagPrefix) + tagPrefix.Length;
					tagLength = htmlfs.IndexOf(tagSuffix, tagIndex) - tagIndex;
					if (tagIndex > 0 && tagLength > 0) {
						return htmlfs.Substring(tagIndex, tagLength);
					}
					else {
						return null;
					}
