...
namespace GameManager {
    ...
    public abstract class DLSitePage {
        ...
        public virtual string GetCategory() {
            if (html != null) {
                // EXAMPLE VOICE WORK CATAGORY <a href="https://www.dlsite.com/maniax/works/type/=/work_type/SOU/from/icon.work"><span class="icon_SOU" title="Voice / ASMR">Voice / ASMR</span></a>
                string tagPrefix = "title=\"";
				string tagSuffix = "\">";
				int tagIndex = html.IndexOf("/work_type/");				//Skip to right line in html code
                tagIndex = html.IndexOf(tagPrefix, tagIndex) + tagPrefix.Length;        // Set tagIndex to after the tagPrefix index
        ...
        public virtual string GetTags() {
            if (html != null) {
                ...
                while (tagIndex != -1) {
                    ...
                    int tagLength = html.IndexOf("</a", tagIndex) - tagIndex;
					if (tagLength > 0) {
                        tags.Add(html.Substring(tagIndex, tagLength).HtmlDecode());
					}						
                    //Find next tag
