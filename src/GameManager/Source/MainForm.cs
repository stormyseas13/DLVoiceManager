...
namespace GameManager {
    public partial class MainForm : Form {
        ...
        /// <summary>
        /// Decrypts an RPG maker MV images asynchronously.
        /// </summary>
        private Task DecryptRpgMakerMVAsync(string gameFolder)
        {
            
            int filesExtracted = 0;
            int fraction;
            byte[] header = new byte[] { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52 };
            byte[] iend = new byte[] { 0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60, 0x82 };
            byte[] key = new byte[16];
            byte[] fragment = new byte[16];
            byte[] fragmentEnd = new byte[8];
            byte[] data;
            byte[] dataDecrypted;
            ...
            var extractor = Task.Factory.StartNew(() => {
                ...
                {
                    var files = IOUtility.EnumerateFilesSafely(gameFolder, "*.rpgmvp", System.IO.SearchOption.AllDirectories);
                    int size = files.Count();
                    foreach (string file in files)
                    {
                        data = File.ReadAllBytes(file);
                        Array.Copy(data, data.Length - 8, fragmentEnd, 0, 8);
                        

                        if (fragmentEnd.SequenceEqual(iend))
                        {
                            dataDecrypted = new byte[data.Length - 16];
                            Array.Copy(header, 0, dataDecrypted, 0, 16);
                            Array.Copy(data, 32, dataDecrypted, 16, data.Length - 32);
                            File.WriteAllBytes(file.Replace(".rpgmvp", ".png"), dataDecrypted);
                            Array.Copy(data, 16, fragment, 0, 16);
                            filesExtracted++;
                            fraction = (int)(filesExtracted / size * 100);
                            StatusBarText = String.Format("({1} files decrypted) {0}", file, filesExtracted);
                        }
                    }
                    if (filesExtracted > 0)
                    {
                        key = IOUtility.baXOR(header, fragment);
                        files = IOUtility.EnumerateFilesSafely(gameFolder, "*.rpgmvo", System.IO.SearchOption.AllDirectories);
                        size += files.Count();
                        foreach (string file in files)
                        {
                            data = File.ReadAllBytes(file);
                            Array.Copy(data, 16, fragment, 0, 16);
                            fragment = IOUtility.baXOR(key, fragment);
                            dataDecrypted = new byte[data.Length - 16];
                            Array.Copy(fragment, 0, dataDecrypted, 0, 16);
                            Array.Copy(data, 32, dataDecrypted, 16, data.Length - 32);
                            File.WriteAllBytes(file.Replace(".rpgmvo", ".ogg"), dataDecrypted);                            
                            filesExtracted++;
                            StatusBarText = String.Format("({1} files decrypted) {0}", file, filesExtracted);
                        }
                        files = IOUtility.EnumerateFilesSafely(gameFolder, "*.rpgmvm", System.IO.SearchOption.AllDirectories);
                        size += files.Count();
                        foreach (string file in files)
                        {
                            data = File.ReadAllBytes(file);
                            ...
                            Array.Copy(fragment, 0, dataDecrypted, 0, 16);
                            Array.Copy(data, 32, dataDecrypted, 16, data.Length - 32);
                            File.WriteAllBytes(file.Replace(".rpgmvm", ".m4a"), dataDecrypted);
                            filesExtracted++;
                            StatusBarText = String.Format("({1} files decrypted) {0}", file, filesExtracted);
                        }
                    }
                }
                StatusBarText = "Completed. Decrypted " + filesExtracted.ToString() + " files";
            });
            ...
            extractor.ContinueWith(task => Invoke(new MethodInvoker(() => {
                ...
                {
                    Logger.LogException(task.Exception.InnerException);
                    MessageBox.Show(this, "Failed to decrypt " + gameFolder + ". See " + Settings.RelativeLogPath + " for details.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            })), TaskContinuationOptions.OnlyOnFaulted);
        ...
        /// Decrypts an RPG maker MV images asynchronously.
        /// </summary>
        private Task DecryptRpgMakerMVEnigmaAsync(string gameFolder)
        {
            
            string lastline;
            int t;
            string enigma = AppDomain.CurrentDomain.BaseDirectory + @"3rdParty\EnigmaVBUnpacker.exe";
            string log = AppDomain.CurrentDomain.BaseDirectory + @"!unpacker.log";
            string log2;
            string filedata;
            FileStream logdata;
            string fileUnp;
            ...
            var extractor = Task.Factory.StartNew(() => {
                if (Directory.Exists(gameFolder))
                {
                    ...
                    {
                        MessageBox.Show("EnigmaVBUnpacker.exe is not found in \\3rdParty folder");
                        return;
                    }
                    foreach (string file in IOUtility.EnumerateFilesSafely(gameFolder, "*.exe"))
                    {
                        FileStream fileStream = File.Open(file, FileMode.Open, FileAccess.Read);
                        BinaryReader binaryReader = new BinaryReader(fileStream);
                        byte[] chunk = binaryReader.ReadBytes(1024);
                        int pos = IOUtility.GetPosition(chunk, ASCIIEncoding.ASCII.GetBytes("enigma"), 0);
                        fileStream.Close();
                        if (pos > 1)
                        {
                            StatusBarText = "Ectracting...";
                            archivePath = file;
                            Process.Start(enigma, "/nogui \"" + file + "\"");
                            log2 = Path.GetDirectoryName(file) + @"\!unpacker.log";

                            lastline = "";
                            t = 0;
                            while (!lastline.Contains("Finished!") || t > 120)
                            {
                                if (File.Exists(log))
                                {
                                    logdata = new FileStream(log, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                                    StreamReader sr = new StreamReader(logdata);
                                    long offset = sr.BaseStream.Length - 100;
                                    ...
                                    if (lastline.Contains("There is not enough space in working directory. Unpacking would most likely fail!"))
                                    {
                                        MessageBox.Show("Enigma VB Unpacker reports not enough space in working directory");
                                        File.Delete(log);
                                        return;
                                    }
                                }
                                ...
                                if (File.Exists(log2))
                                {
                                    logdata = new FileStream(log2, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                                    StreamReader sr = new StreamReader(logdata);
                                    long offset = sr.BaseStream.Length - 100;
                                    sr.BaseStream.Seek(offset, SeekOrigin.Begin);
                                    while (!sr.EndOfStream)
                            ...
                            {
                                File.Delete(log2);
                                fileUnp = Path.Combine(Path.GetDirectoryName(file), Path.GetFileNameWithoutExtension(file) + @"_unpacked.exe");
                                if (File.Exists(fileUnp))
                                {
                                    File.Delete(fileUnp);
                                }
                                default_folder = Path.GetDirectoryName(file) + @"\%DEFAULT FOLDER%";
                                if (Directory.Exists(default_folder))
                                {
                                    FileSystem.MoveDirectory(default_folder, Path.GetDirectoryName(file),true);
                                }
                            }
                        }
                    }
            ...
            extractor.ContinueWith(task => Invoke(new MethodInvoker(() => {
                ...
                if (File.Exists(archivePath))
                {
                    if (Settings.Instance.PostCgExtractionAction == PostExtractionAction.Ask)
                    {
                        var message = "Delete the successfully extracted executable \"" + archivePath + "\"?\nThe game will still run after the executable is deleted.";
                    ...
                    if (action == PostExtractionAction.Delete)
                    {
                        ...
                        {
                            Logger.LogException(e);
                            StatusBarText = "Failed to delete the successfully extracted executable.";
                        }
                    }
                    else if (action == PostExtractionAction.Rename)
                    {
                        ...
                        {
                            File.Move(archivePath, Path.Combine(Path.GetDirectoryName(archivePath),
                                                                Path.GetFileNameWithoutExtension(archivePath) + ".ex_"));
                        }
                        catch (Exception e)
                        {
                            Logger.LogException(e);
                ...
                Task.Factory.StartNew(() => {
                    foreach (var game in selectedObjects) {
                        ...
                        if (game.Path == null) {
                            ...
                            if (Directory.Exists(gameFolder)) {
                                foreach (string file in IOUtility.EnumerateFilesSafely(gameFolder, "*", System.IO.SearchOption.AllDirectories)) {
                            ...
                            if (archivePath == null) {
                                ...
                                else if(archivePath == "MV")
                                {
                                    DecryptRpgMakerMVAsync(gameFolder).Wait();
                                }
                                else if (archivePath == "EnigmaMV")
                                {
                                    DecryptRpgMakerMVEnigmaAsync(gameFolder).Wait();
                                }
                                else {
        ...
        private class TileViewRenderer : AbstractRenderer {
            ...
            public TileViewRenderer(ObjectListView listView) {
                ...
                var c = listView.SelectedBackColorOrDefault;
